...
Also Pending are feedback maturity, aggregation floor and retention, sample sizes and duration, provider-specific authoritative evidence mappings that can resolve Unknown to Success or Failed, numeric meaning of “promptly,” opt-out attribution beyond the direct flow, seen state, severity ordering, lock-screen privacy, remote token lifecycle, and the quarantined Task 6 quiet-period proposal. The conservative Unknown lock and quiet-proposal prohibition are governed. No measure may invent a resolver, quiet behavior, value, or use an immature choice as a Pass.
