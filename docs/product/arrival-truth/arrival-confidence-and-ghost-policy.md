...
The Due timer measures uninterrupted time since the arrival first became due without observed movement or stop progress. It does not replace feed age or train movement age.

| No-progress time after Due begins | Required disposition |
|---|---|
| From 0 through exactly 60 seconds | **Due** may remain only while movement evidence is still fresh and every Live gate continues to pass. Sixty seconds is the maximum, not a target duration. |
| Strictly more than 60 seconds through exactly 120 seconds | Freeze immediately as **Holding**. Remove it from the primary next-three and retain it as a separate held-train warning or secondary status. |
| Strictly more than 120 seconds | The exact arrival event cannot appear in or return to the primary next-three. Retain train-level held context in the secondary area while the entity and exact stopping pattern remain valid. Apply the movement-age state: Holding through exactly 180 seconds, then Uncertain beyond 180 seconds. |

At exactly 60 seconds, Due is still within the permitted maximum if movement remains fresh. The first instant strictly after 60 seconds is Holding. At exactly 120 seconds, the train remains secondary Holding context with no advancing or exact minute. The first instant strictly after 120 seconds removes any remaining exact-event treatment; it does not silently delete the valid train instance.
