...
| Supplemented GTFS | Preferred planned-service baseline; planned-pattern validation; future journey planning outside the real-time horizon; first clearly labeled schedule fallback during a genuine real-time outage. A usable applicable edition is the sole positive schedule source inside its declared route/service-date/effective-time/horizon mask, even when a desired regular occurrence is absent. | It cannot produce a live countdown, restore a missing real-time trip or stop call, override a live or negative-evidence conflict, or prove service is normal or complete. Its omission blocks regular backfill inside the mask but does not by itself prove cancellation, bypass, or no service. Hourly publication and “most, not all” changes for the next seven calendar days are limits, not guarantees. |
| Regular static GTFS | Normal service schedule; stable route and station relationships; long-range planning beyond the supplemented horizon; last-resort clearly labeled schedule fallback only when no usable supplemented coverage mask applies to the requested scope and time. | It cannot produce a live countdown, fill a stop missing from a live remaining-stop sequence, restore a trip absent from a healthy full real-time snapshot, override a veto, prove current normal service, or resurrect an occurrence or stop omitted inside a usable supplemented mask. |
...
- Supplemented-versus-regular selection is scope-first, not occurrence-first. Coverage is established without requiring the desired trip or stop to appear. Inside a usable supplemented mask, regular GTFS supplies no positive departure; outside every usable mask, regular GTFS may be evaluated as the last schedule fallback.
