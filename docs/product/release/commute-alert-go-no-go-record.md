...
|---|---|
| Source sections | Approved specification §§25.2–25.7, 26.3, 28.1–28.3, 30, 32.3, 33.5, and 34; commute alerts and launch quality plan Task 10; accepted immutable Commute Tasks 1–9 and fixed gate package |
...
| `CG-BLK-06` | Provider-specific authoritative delivery-resolution evidence, seen-state interpretation, severity handling, correction delivery, token handling, lock-screen privacy, and quiet-proposal boundaries | Resolver mappings, Seen/severity/correction/token/lock-screen evidence remain unresolved; the conservative Unknown lock is governed, and Task 6’s 900-second quiet proposal is quarantined, non-executable, and not a gate input | Blocks complete delivery, privacy, correction, and nuisance evidence | Resolver evidence and every other behavior/evidence gap are fixed and all mandatory scenarios Pass without adopting the quarantined proposal |
