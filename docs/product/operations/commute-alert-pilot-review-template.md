...
| `OPS-N02`; `OPS-N02-SRC-v1` | Final Send attempt returns Unknown acknowledgment | All Off → Commander holds dependent episode action pending approved rule → episode On | Preserve Unknown; no retry, duplicate assumption, correction, or replay; saved state and boards unchanged | Release future current evaluation only after acknowledgment semantics and required approvals; old item never replayed | Assume Failed/Success, retry, send correction, or count deterministic miss/duplicate |
...
Roster names/pages/backups/handoff/access tests, switch implementation, correction/retraction class/channel/template, delivery acknowledgment, seen state, severity ordering, numeric “promptly,” sample size/duration/floor/retention, active-commuter definition, feedback maturity, opt-out window, remote token lifecycle, lock-screen behavior, and the Task 6 quiet-period proposal remain Pending. No drill invents them.
