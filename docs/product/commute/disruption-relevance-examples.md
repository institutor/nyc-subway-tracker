...
| `REL-20`; `REL-20-SRC-v1` | Two independent overlapping windows receive the same episode and materially equivalent impact/action | Both evaluate; equivalent duplicate branch Fails for extra candidate → **Suppress** extra candidate | At most one notification candidate; preserve both windows | Two equivalent pushes; merge or rewrite windows |
