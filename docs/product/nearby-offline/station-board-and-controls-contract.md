...
The Draft [artifact index](../artifact-index.md) row now aligns with this contract's §§14.6, 15, and 16 provenance, full Task 5 plan provenance, and Product, Accessibility, Data Quality, and Content review path. That metadata alignment is not approval and does not advance this artifact beyond **Draft**; all reviewer decisions and observed station-board evidence remain **Pending**.
...
| Accessible-path and equipment state | Draft companion accessibility artifacts under `docs/product/accessibility/` | Preserve exact path scope and fail closed; do not infer a badge or current path. | Same-version approval and observed evidence Pending |
| Platform and positioning guidance | Draft companion guidance artifacts under `docs/product/guidance/` | Show only verified supplied state and guidance; suppress during conflict or ambiguity. | Same-version approval and observed evidence Pending |
