...
3. Select the newest validated, non-superseded-for-the-claim, departure-eligible supplemented edition that covers the proposed operating service date and departure time.
4. If no supplemented edition passes every eligibility gate—because supplemented data is unavailable; no applicable validated edition exists; the candidate is expired or outside effective coverage or horizon; service-date or departure coverage is missing; timestamp evidence is unusable or timestamp-quarantined; the edition is superseded for the claim without a newer eligible overlap; or it is **Topology only**—evaluate eligible regular GTFS for the same operating service date and proposed departure as the next candidate.
5. If regular GTFS cannot support that service-date and departure claim, show no timed journey rather than inventing service.
6. Keep one itinerary across a Typical-weekday-to-Late-night or Late-night-to-Typical-weekday transition when an eligible timed pattern exists.
7. Mark the exact leg, station, transfer, or time boundary where the service pattern changes and explain the consequence for later legs.
8. Show the layer-specific **reference** and **Not current** treatment for every reference-derived segment and retain the source owner's confidence, currency, effective-date, and service-date treatment.

Supersession remains claim-scoped: a later validated edition displaces an earlier edition only inside their overlapping effective and service-date coverage, and an earlier edition outside that overlap may remain the newest eligible supplement only when it passes every other gate. Current active or effective vetoes continue to control at their supported scope. Regular GTFS is reference evidence, never Live or current, and cannot override a veto.
...
| Future journey with an eligible supplemented edition | Newest validated, non-superseded-for-the-claim, departure-eligible supplemented edition covering the proposed operating service date and departure time | Show reference meaning, effective horizon/service date, and owner-supplied confidence and currency labels; apply every active or effective current veto whose supported scope overlaps the proposed leg | Live implication, complete-change guarantee, use outside coverage, cherry-picking an older superseded edition inside overlap, or using a **Topology only** or timestamp-quarantined edition for a timed departure |
| Future journey after supplemented eligibility failure | Eligible regular GTFS for the same operating service date and proposed departure, evaluated as the next candidate when supplemented data is unavailable; no applicable validated edition exists; the candidate is expired or outside effective coverage or horizon; service-date or departure coverage is missing; timestamp evidence is unusable or timestamp-quarantined; the edition is superseded for the claim without a newer eligible overlap; or it is **Topology only** | Show reference meaning and owner-supplied confidence/currency limitation for the proposed service date; preserve the supplement owner's claim-scoped supersession result and every active or effective current veto | Live implication, a current-service claim, regular GTFS overriding a veto, or returning to an earlier superseded overlapping supplement for a more optimistic trip |
...
| Pattern decision | Determine the pattern from the trip's departure, arrival, service date, and exact supported boundary. Select the newest validated, non-superseded-for-the-claim, departure-eligible supplemented edition covering the proposed operating service date and departure time. If supplemented data is unavailable; no applicable validated edition exists; the candidate is expired or outside effective coverage or horizon; service-date or departure coverage is missing; timestamp evidence is unusable or timestamp-quarantined; the edition is superseded for the claim without a newer eligible overlap; or it is **Topology only**, evaluate eligible regular GTFS for the same service date and departure as the next candidate. If regular GTFS cannot support the claim, show no timed journey rather than inventing service. |
...
| Prohibited result | Splitting the trip into unrelated itineraries; switching on phone clock or midnight alone; using theme as service meaning; duplicating or reordering a leg; hiding the boundary; losing service-date meaning; selecting a superseded edition inside overlapping coverage; using **Topology only** or timestamp-quarantined data for a timed departure; presenting regular GTFS as Live or current; letting regular GTFS override an active or effective veto; or inventing a timed journey when neither source is eligible. |
| Required reviewers | Product, Accessibility, Data Quality, Content |
