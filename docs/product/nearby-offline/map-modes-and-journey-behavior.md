...
| **Typical weekday** | Explicit daytime reference pattern supplied by the governed schedule/topology source. It is not evidence of current service, alerts, reroutes, closures, arrivals, or equipment operation. | Online, show **Typical weekday — reference** and a visible **Not current** state wherever the map, route detail, station context, or journey could be mistaken for current. When the [offline content and validity contract](offline-content-and-validity-contract.md) admits the stored layer offline, apply its exact **Reference pattern—not live.** throughout the associated map context. | Do not use **Actual now**, Live styling, current disruption absence, or unlabeled cached geometry. Task 8 owns storage, offline-open eligibility, and exact offline copy. |
| **Late night** | Explicit overnight reference pattern supplied by the governed schedule/topology source. It is not selected merely because the phone clock is late. | Online, show **Late night — reference** and a visible **Not current** state wherever the map, route detail, station context, or journey could be mistaken for current. When the [offline content and validity contract](offline-content-and-validity-contract.md) admits the stored layer offline, apply its exact **Reference pattern—not live.** throughout the associated map context. | Do not imply current overnight service, silently replace a weekday pattern, or infer live equipment or disruption state. Task 8 owns storage, offline-open eligibility, and exact offline copy. |

Online, the layer-specific reference name and **Not current** state stay visible in the layer control and map legend and remain associated with any route detail or future journey built from that reference geometry. Offline, Task 8's exact **Reference pattern—not live.** remains associated with the stored layer and its map, route, station, and journey context. Opening a contextual station board from a reference map does not convert the map to Actual now or convert reference geometry into current truth. The station board presents only its separately governed evidence states while the underlying reference treatment remains visible.
...
3. Establish supplemented ownership independently of whether the desired trip or stop appears. Select the newest validated, non-superseded-for-the-claim, currency-eligible supplemented edition whose declared route or feed-group, service-date, effective-time, horizon, direction or operational-axis, and other supplied scope contains the proposed segment and time.
4. Inside that usable supplemented mask, build a timed segment only from an explicit coherent supplemented occurrence and stopping pattern. If the desired regular occurrence or stop is absent, show no timed journey for that alternative; do not fill it from regular GTFS and do not infer cancellation, bypass, or no service from omission alone.
5. Evaluate eligible regular GTFS only when no usable supplemented mask applies—because supplemented data is unavailable; no applicable validated edition exists; the request is outside every applicable effective interval, service-date coverage, or horizon; timestamp evidence is unusable or quarantined; the only applicable edition is expired or **Topology only**; or an earlier edition remains superseded inside an overlap without a newer usable supplemented owner.
6. If the eligible source cannot support the service-date and departure claim, show no timed journey rather than inventing service.
7. Keep one itinerary across a Typical-weekday-to-Late-night or Late-night-to-Typical-weekday transition when an eligible timed pattern exists.
8. Mark the exact leg, station, transfer, or time boundary where the service pattern changes and explain the consequence for later legs.
9. Show the layer-specific **reference** and **Not current** treatment for every reference-derived segment and retain the source owner's confidence, currency, effective-date, and service-date treatment.

Supersession remains claim-scoped: a later validated edition displaces an earlier edition only inside their overlapping effective and service-date coverage, and an earlier edition outside that overlap may remain the newest eligible supplement only when it passes every other gate. Supplemented ownership is scope-first, never a per-occurrence merge with regular service. Current active or effective vetoes continue to control at their supported scope. Regular GTFS is reference evidence, never Live or current, and cannot override a veto or resurrect an omitted occurrence inside a usable supplemented mask.
...
On a reference layer, structural accessibility information remains reference context and cannot imply current equipment operation. The [offline content and validity contract](offline-content-and-validity-contract.md), Task 8, owns stored accessibility eligibility and its conditional offline wording while preserving every route-critical operational equipment state as Unknown. This contract never manufactures **Accessible now**.
...
After every applicable owner decision and all six rider-relevant factors tie exactly, use ascending stable canonical journey-alternative identity as a neutral final presentation tie-break. When Accessible Route Only is On, the map consumes the companion accessibility owner's stable canonical alternative or path identity and exact final order; it does not derive a replacement identity or reorder that accessibility result. The identity's unique normalized comparison form must be fixed before ranking and remain stable across evidence refreshes and copy changes for the same alternative.

This identity order exists only to make primary selection, bounded-list cutoff, and repeated presentation stable. It does not imply that one fully tied journey is better, shorter, safer, more resilient, more accessible, faster, more current, or more trustworthy. Source order, retrieval order, response order, wrapper order, record enumeration, render order, localized label order, and prior screen position are never tie-breakers. If the required canonical identity is missing, duplicate, mutable, or conflicting, do not promote one tied option as the governed primary until its owner corrects the decision package.
...
| Future journey inside a usable supplemented mask | Newest validated, non-superseded, currency-eligible supplemented coverage owner for the route/scope, operating service date, effective time, and horizon; an explicit coherent supplemented occurrence is still required for a timed segment | For an explicit occurrence, show reference meaning, effective horizon/service date, and owner-supplied confidence and currency labels and apply every overlapping veto. For an omitted occurrence or stop, show no timed journey for that alternative and make no unsupported negative claim. | Live implication, complete-change guarantee, use outside coverage, cherry-picking an older superseded edition, regular-GTFS resurrection of an omitted covered occurrence or stop, or timed use of a **Topology only** or timestamp-quarantined edition |
| Future journey when no usable supplemented mask applies | Eligible regular GTFS for the same operating service date and proposed departure, evaluated only when supplemented data is unavailable; no applicable validated edition exists; the request is outside effective coverage, service-date coverage, or horizon; timestamp evidence is unusable or quarantined; the only applicable edition is expired or **Topology only**; or an earlier edition remains superseded inside an overlap without a newer usable owner | Show reference meaning and owner-supplied confidence/currency limitation for the proposed service date; preserve the supplement owner's claim-scoped supersession result and every active or effective current veto | Live implication, a current-service claim, per-occurrence fallback inside a usable supplemented mask, regular GTFS overriding a veto, or returning to an earlier superseded overlapping supplement for a more optimistic trip |
| No valid timed pattern | Neither a supplemented edition nor regular GTFS can support the proposed operating service-date and departure claim | Show no timed journey and offer only evidence-supported changes to destination, time, or mode | Guessed topology, invented service, inferred timing, or a weaker unlabeled itinerary |

Supplemented-edition supersession remains claim-scoped to overlapping effective and service-date coverage; outside that overlap, an earlier edition may remain the newest eligible supplement only when it passes every other gate. A usable supplemented mask remains the sole positive schedule source inside its scope even when a desired regular occurrence is absent, while that absence proves no cancellation or bypass. A current active or effective veto applies only at its supported route, direction, station or segment, and time scope; its absence never proves normal future service. Neither supplemented nor regular schedules create a Live stop call, current equipment state, or **Accessible now** claim.
...
| Walkthrough field | Expected result |
|---|---|
| Starting state | A future trip has a source-supported operating service date, New York local departure and arrival times, owner-supplied supplemented and regular schedule candidates, any active or effective current vetoes, and a governed weekday-to-late-night boundary during the trip. Appearance, spatial view, overlays, origin, and destination are already selected. |
| Rider action | Plan the trip across the supplied service-pattern boundary. |
| Pattern decision | Determine the pattern from the trip's departure, arrival, service date, and exact supported boundary. Establish the newest usable supplemented coverage owner independently of occurrence presence. Inside its mask, use only explicit coherent supplemented occurrences and show no timed alternative when the desired regular occurrence or stop is omitted. Evaluate regular GTFS only when no usable supplemented mask applies because the source is unavailable, invalid, quarantined, expired, **Topology only**, superseded without a newer usable owner, or the request is outside its effective/service-date/horizon coverage. If the eligible source cannot support the claim, show no timed journey rather than inventing service. |
| Expected itinerary | When an eligible timed pattern exists, keep one itinerary. Identify the exact leg, station, transfer, or supported time where service changes from Typical weekday to Late night; explain the consequence for later legs; preserve route/destination/direction and service order; preserve claim-scoped supersession and every applicable current veto; show the layer-specific **reference** and **Not current** treatment plus the owner-supplied confidence/currency and service-date context. Regular GTFS remains reference evidence, never Live or current. |
...
| Prohibited result | Splitting the trip into unrelated itineraries; switching on phone clock or midnight alone; using theme as service meaning; duplicating or reordering a leg; hiding the boundary; losing service-date meaning; selecting a superseded edition inside overlapping coverage; using **Topology only** or timestamp-quarantined data for a timed departure; resurrecting an occurrence or stop from regular GTFS inside a usable supplemented mask; presenting regular GTFS as Live or current; letting regular GTFS override an active or effective veto; or inventing a timed journey when the owning source has no eligible occurrence. |
| Required reviewers | Product, Accessibility, Data Quality, Content |
...
| Reroute, scenario 25, and supplemented-coverage observations | `docs/product/nearby-offline/acceptance-evidence.md`, Task 14; Release Quality Lead | Evidence ledger present; observed evidence Not run — Pending |
...
| Do both reference layers carry unmistakable layer-specific reference and **Not current** meaning online while Task 8's exact **Reference pattern—not live.** remains visible throughout eligible offline map context? | Required by the linked contracts; not rendered. | Task 7 online Typical-weekday, Late-night, route-detail, station-context, and journey captures; Task 8 offline map-open, copy, and eligibility evidence |
...
| Can shuffled source or retrieval order change the primary journey, accessible alternative, or bounded-list membership when all rider-relevant values tie? | No; only the owner-supplied stable canonical identity controls the neutral final order, without a superiority claim. | MAP-GEO01 together with accessibility fixture ARO-15, repeated with identical decisions and shuffled inputs |
...
| Can a regular-GTFS occurrence or stop reappear inside a usable supplemented coverage mask merely because the supplement omits it? | No; omission yields no timed alternative and no unsupported negative claim. | MAP-COV01 and Arrival Truth FB6 |
