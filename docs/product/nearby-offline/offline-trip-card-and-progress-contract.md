...
Task 10 will own the persistent global offline message, cached-current presentation, preserved-screen degradation, reconnection order, and active-trip invalidation warning. Task 12 will own retention, deletion, reset, synchronization, analytics, diagnostic separation, and broader personal-data rules. Task 14 will own observed Nearby/offline acceptance evidence. Those artifacts are absent, as are companion complete-path, equipment, exit, platform-zone, transfer, and contingency artifacts; this Draft records conditional consumption, not their approval.
...
Reopening, foregrounding, reading, scrolling, or completing the card likewise cannot refresh a claim or reset manual progress. Task 10 may later define reconnection behavior, but reconnection cannot use manual progress as operational evidence.
...
| Complete accessible path, Accessible Route Only, and equipment state | Companion accessibility owners | Consume exact path, direction, machine, status, and warning; fail closed while artifacts and evidence are absent |
...
| Persistent offline message, cached-current presentation, preserved screen, reconnection, and active-trip invalidation warning | `docs/product/nearby-offline/offline-degraded-and-reconnection-states.md`, Task 10 | Planned owner; this contract creates no global message or recovery sequence |
| Retention, deletion, reset, synchronization, analytics, diagnostics, and broader personal-data rules | `docs/product/nearby-offline/location-and-personal-data-rules.md`, Task 12 | Planned owner; this contract records no policy for those decisions |
| Observed Nearby/offline cases | `docs/product/nearby-offline/acceptance-evidence.md`, Task 14 | Planned owner; all expected walkthroughs remain **Not run — Pending** |
...
| Connectivity change | Service is lost before boarding. The card opens without an account or network wait; Task 10's global offline and preserved-screen behavior remains a later dependency. |
...
| Derived accessible-trip walkthrough | TRIP-A01 retains the complete structural chain while operational equipment is Unknown and manual progress remains non-operational | **Not run — Pending**; companion artifacts are absent |
...
| Are exit, platform-zone, accessible-path, and contingency fields admitted only from exact eligible verified owner records? | Yes by expected contract; companion artifacts are absent. | Companion approval and fixed conditional-module cases |
...
| Does this contract define retention, tracking, synchronization, analytics, diagnostics, Task 10 recovery, or companion truth? | No. | Cross-artifact review after Tasks 10 and 12 and companion artifacts exist |
