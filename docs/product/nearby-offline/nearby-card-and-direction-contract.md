...
Task 4 applies no additional geographic or personalization tie-break at the three-card cutoff. Task 3's final order already includes Task 11's deterministic baseline-first-three promotion cohort and excludes any saved candidate that would hide a closer result or exceed three cards. Task 4 preserves **About the same walk.** and **One of the closest confirmed entrances.** when supplied, and never turns a neutral presentation order into a uniquely-nearest claim. It must not duplicate a complex, widen an entrance or direction relationship, relax a Task 3 exclusion, pull a lower complex upward because it has more arrival rows, or replace an honest smaller result set. Arrival quantity, Live state, disruption state, screen fit, and desire to fill three cards never change the station order.
