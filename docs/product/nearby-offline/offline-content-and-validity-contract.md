...
Task 9 will own active offline trip-card content, last-checked treatment, and manual progress. Task 10 will own the persistent global offline message, preserved-screen degradation, reconnection priority, and recovery behavior. Task 14 will own observed Nearby/offline acceptance evidence. None of those planned artifacts exists, and this contract does not pre-approve their decisions.

This artifact is **Draft**. The authoritative [Gate 0 exit record](../quality/gate-0-exit-record.md) remains **NO-GO — GATE 0 NOT PASSED**; public boards blocked. Companion complete-path, Accessible Route Only, equipment, and accessibility-copy artifacts are absent. Tasks 9, 10, and 14 are absent. Schedule truth remains Draft/Pending, both map-open paths and all five cases below are **Not run — Pending**, and map and brand rights are unresolved. Expected contract prose is not observed evidence or public-release permission.

### Provenance reconciliation

The [Approved product artifact index](../artifact-index.md) cites specification §§18.1, 18.4, 31.4, and 31.8 for Task 8. That row incorrectly assigns §18.4 reconnection to this artifact. Task 10 owns reconnection, while Task 8 consumes §18.3 for offline map copy, schedule-age presentation, and accessibility wording. Record the corrected provenance before any lifecycle advancement. This task does not edit the index; reconciliation remains **Pending**.
...
| Saved trip cards | The last device-held card and its separately governed content | Open the saved card without a connection | Task 9 owns completeness, last-checked treatment, contingencies, and manual progress; inventory presence is not freshness |
...
- A valid Current or Stale supplemented record may add a scheduled clock time only for its exact covered departure and subject to every veto; only that result may be labeled **Reference itinerary**.
...
Task 9 owns the captured active trip card and the freshness of last-checked service, equipment, guidance, and contingencies. This contract owns eligibility for a newly planned offline **Reference itinerary** and the weaker untimed structural-route or structural-path result when no eligible timed schedule exists.
...
Companion complete-path, Accessible Route Only, equipment, and accessibility-copy artifacts remain absent, so this Draft defines expected consumption only and demonstrates no positive structural or current accessibility result.
...
| Current schedule, Stale reference, Topology only, quarantine, currency boundaries, and **Stored schedule—service changes may differ** | [Schedule fallback and currency policy](../arrival-truth/schedule-fallback-and-currency-policy.md), Arrival Truth | Consume exactly; do not rename, strengthen, recompute from retrieval age, or apply to an ineligible claim |
| Persistent global offline message, cached-current presentation, preserved screen, and reconnection | `docs/product/nearby-offline/offline-degraded-and-reconnection-states.md`, Task 10 | Planned owner; this artifact does not create persistent global copy or a reconnection sequence |
| Active offline trip-card freshness, last-checked service/equipment, contingencies, and manual progress | `docs/product/nearby-offline/offline-trip-card-and-progress-contract.md`, Task 9 | Planned owner; saved-card presence in the inventory does not establish freshness or completeness |

## Scenario traceability and pending evidence

| Source scenario or case | Expected Task 8 application | Current evidence |
|---|---|---|
| §31.4 scenarios 23–25 | Task 8 supplies instant reference-map and new-reference-itinerary validity boundaries; Task 10 owns the persistent tunnel/offline state, Task 9 owns saved-trip behavior, and Task 7 owns the day-to-night pattern explanation | **Not run — Pending**; Tasks 9, 10, and 14 are absent |
...
| Scenario 51 | Stored trip cards are inventoried here, but Task 9 owns the Release 1 minimum-complete card without unavailable exit or platform-zone guidance | **Not run — Pending**; Task 9 and Task 14 are absent |
...
| Are Task 9 trip-card freshness and Task 10 persistent offline/reconnection behavior left with their planned owners? | Yes. | Cross-artifact review after Tasks 9 and 10 exist |
