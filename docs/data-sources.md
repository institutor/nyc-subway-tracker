...
An admitted shadow claim always carries a compact proof bound to that comparison's earlier record and prior claim: prior observation/path, the vehicle's exact current stop ID and source sequence, current movement evidence, future target event, eligible track, canonical trip/start-date/start-time/train service ownership, issued service-decision binding, and the exact admitted stop call. The vehicle-owned stop call must be the claim's next call, including repeated-stop occurrence. A record with no comparison context cannot contain an admitted claim. Accepted alert context records its exact current/stale state using the shared ten-minute threshold and a context identity recomputed from the canonical alert source row; stale or failed alerts cannot support eligible service. The potentially large issued alert identity is not stored; only its SHA-256 commitment is bound into the compact service-decision proof.
