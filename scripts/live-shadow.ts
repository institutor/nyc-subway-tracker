...
import { projectShadowClaims, type ShadowClaim } from '../src/server/services/shadow-validation';
...
async function main(): Promise<void> {
  ...
  const recordId = `shadow-${recordedAt.replaceAll(':', '-')}`;
  if (dryRun) {
  ...
  for (const source of sources.filter((candidate) => candidate.role === 'subway-realtime')) {
    ...
    claims.push(...projectShadowClaims({ snapshot, alertSnapshot, decisionTime }).slice(0, 500 - claims.length));
    if (claims.length >= 500) break;
  }
  const comparableClaims = claims.filter(({ disposition }) => disposition === 'admitted');
  const comparisons = compareArgument ? await compareProgress(compareArgument, comparableClaims) : [];
  const record = {
    schemaVersion: SHADOW_SCHEMA,
    recordId,
    mode: 'shadow',
  ...
  await writeShadowRecordAtomic(outputPath, `${JSON.stringify(record, null, 2)}\n`);
