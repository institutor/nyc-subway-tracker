...
const request = { complex: 'A12|125 St', constituent: 'A12|125 St (8 Av)', route: 'A', servicePattern: 'ordinary', direction: 'northbound', destination: 'Inwood-207 St', platformId: 'A12N', objectiveType: 'accessible-exit', objectiveTarget: 'EL-A12-01', accessiblePathVersion: 'path-v1', now: new Date('2026-08-01T00:00:00Z'), platformState: 'confirmed' as const, rerouted: false, exposure: { exposed: true, surface: 'validation' as const, packageVersion: 'package-v1', immutable: true, reviewed: true } };
...
describe('immutable platform guidance', () => {
  ...
  test('returns guidance only for exact scope, orientation objective and current platform match', () => {
    ...
    expect(resolvePlatformGuidance([record()], { ...request, exposure: { ...request.exposure, exposed: false } })).toBeUndefined();
  });
