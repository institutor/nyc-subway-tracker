...
export function resolvedWarning(
  ...
  options: { readonly decisionPointId?: string } = {},
) {
  ...
  const progress = acceptAccessibilityJourneyProgress({
    ...
    affectedOrder: 3,
    decisionPoints: options.decisionPointId ? [{
      id: options.decisionPointId,
      order: 2,
      actionPathEvaluationId: selectedPath.evaluationId,
    }] : [],
